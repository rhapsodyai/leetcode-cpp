...
vector<bool> kidsWithCandies(vector<int>& candies, int extraCandies) {
    ...
    /* check and store if it is possible for ith kid to have the max number of candies */
    for(auto& e : candies)
        max > e + extraCandies ? v.push_back(false) : v.push_back(true);
...
int main(int argc, char** argv) {
    ...
    vector<bool> ret = kidsWithCandies(v, 3);
    for(auto e : ret)
        cout << e << ' ';
    cout << endl;
}
